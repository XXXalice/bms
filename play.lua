...
local function main(keysNumber)
	...
	geo.note.white_w = geo.note.original_white_w * geo.note.scale_w - 1
	geo.note.black_w = geo.note.original_black_w * geo.note.scale_w -1
	...
	geo.lane.w = geo.note.scratch_w + geo.note.white_w * 4 + geo.note.black_w * 3 + geo.lane.separateline_w * 7 + 4.5
	...
	do
		...
		if not property.judgeDetail.item.off.isSelected() then
			...
			if property.judgeDetail.item.fastSlow.isSelected() then
				...
				-- カスタマイズ用のタイミングビジュアライザ
				-- table.insert(skin.timingvisualizer,
				-- 	{id = "timingvisualizer"}
				-- )
				-- table.insert(skin.destination, {id = "timingvisualizer", timer = 41, offset = offset.timingvisualizer.id, op = {32}, dst = {
				-- 	{x = x-72, y = y+263, w = w+130, h = h-17}
				-- }})
			-- +-ms
